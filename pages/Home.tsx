...
/* ── Reveal: fade + translate only (no blur — avoids repaint jank) */
...
const staggerItem = {
  ...
  show: { opacity: 1, y: 0, transition: { duration: 0.45, ease: springEase } },
};

/* ── FlowDiagram: proper flowchart with connecting arrows ───────── */
      ...
      {steps.map((s, i) => (
        <React.Fragment key={i}>
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.4, delay: i * 0.055, ease: springEase }}
          ...
          {i < steps.length - 1 && (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.25, delay: i * 0.055 + 0.18 }}
...
function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Portrait card — hidden on mobile to prevent overflow */}
      <div
      ...
      >
          ...
          {words.map((word, wi) => (
            <motion.span key={wi} className="block overflow-hidden">
              <motion.span
                ...
                transition={{ duration: 0.9, delay: 0.3 + wi * 0.12, ease: springEase }}
              >
                {word}
              </motion.span>
            </motion.span>
        ...
        <Reveal delay={0.6}>
          <div className="mt-10 flex flex-wrap items-center gap-3">
            ...
            >
              View my work
              <span className="ml-1 inline-flex h-9 w-9 items-center justify-center rounded-full bg-black/10 transition-all duration-500 group-hover:translate-x-1 group-hover:-translate-y-px">
                <ArrowDown size={14} />
              </span>
            </a>
            <a
            ...
            >
              Download CV
              <span className="ml-1 inline-flex h-9 w-9 items-center justify-center rounded-full transition-all duration-500 group-hover:translate-x-1 group-hover:-translate-y-px"
...
/* ═══════════════════════════════════════════════════════════════
   CASE STUDY CONTENT — reused by modal
   ═══════════════════════════════════════════════════════════════ */
            ...
            <div className="flex items-start justify-between gap-6 px-8 md:px-10 pt-8 pb-6 border-b border-hairline flex-shrink-0">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {project.metrics && (
                    <span className="font-display italic text-sm text-ink font-medium">{project.metrics}</span>
                  )}
                </div>
              ...
              <button
                onClick={onClose}
                aria-label="Close"
                className="flex-shrink-0 mt-1 h-10 w-10 rounded-full border border-hairline flex items-center justify-center text-ink-muted hover:text-ink hover:border-ink/30 transition-colors duration-250"
              >
            ...
            <div className="flex gap-1 px-8 md:px-10 py-4 border-b border-hairline flex-shrink-0">
              {(['overview', 'technical'] as const).map(tab => (
                <button
                  ...
                  className={`px-5 py-2 rounded-full text-xs font-medium capitalize transition-all duration-250 ${
        ...
        >
          {PROJECTS.map((project, idx) => (
            <motion.article
              ...
              whileHover={{ y: -4, transition: { duration: 0.3, ease: springEase } }}
            >
              <div className="bezel-core p-6 md:p-7 flex flex-col h-full">
                ...
                <p className="mt-4 text-sm text-ink/70 leading-relaxed font-normal flex-1">
                  {project.description}
                </p>
                ...
                <div className="mt-6 pt-5 border-t border-hairline">
                  <button
                    ...
                    className="group w-full flex items-center justify-between text-sm font-medium text-ink hover:opacity-70 transition-opacity duration-300"
                  >
                    <span>View case study</span>
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-ink text-white transition-transform duration-300 group-hover:translate-x-0.5">
            ...
            <div className="flex gap-2">
              {[
                ...
                <a key={s.label} href={s.href} target="_blank" rel="noreferrer" aria-label={s.label}
                  className="grid h-12 w-12 place-items-center rounded-full border border-hairline bg-white text-ink-muted hover:text-ink transition-colors duration-300">
                  {s.icon}
                </a>
              ))}
            </div>
