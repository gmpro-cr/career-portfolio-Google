...
export const PROJECT_EXTRAS: Record<string, ProjectExtra> = {
  ...
  'ai-persona-interaction-platform': {
    ...
    userPersona: {
      ...
      painPoint: '"I\'ve watched every Elon Musk interview. I follow Naval on Twitter. But I can\'t actually ask them about my specific situation, like whether to leave my stable job for a startup. I just want 20 minutes with someone whose thinking I trust."',
    },
    ...
    journey: [
      ...
      { phase: 'Onboard',   action: 'Starts first conversation with chosen persona. Guest mode, no signup required',      emotion: 'Cautious'   },
  ...
  'ai-credit-intelligence-platform': {
    ...
    journey: [
      { phase: 'Upload',     action: 'Analyst uploads borrower\'s annual report PDF (100+ pages). System immediately starts extraction with no configuration', emotion: 'Hopeful'     },
    ...
    roadmap: [
      ...
      { phase: 'Enterprise Polish', status: 'shipped', quarter: 'Jan 2026', items: ['Claude API cross-validation layer', 'Excel export (OpenPyXL) in bank template format', 'PDF export', 'Karpathy autonomous web research loop', 'Confidence scoring per section (High/Medium/Low)'] },
  ...
  'automated-job-discovery-agent': {
    ...
    journey: [
      ...
      { phase: 'Schedule',   action: 'Agent runs via GitHub Actions cron twice daily. No server running 24/7, so no infrastructure cost', emotion: 'Hands-off'   },
  ...
  'ai-engineering-field-guide': {
    ...
    journey: [
      ...
      { phase: 'Ask',      action: 'Opens "Ask the book" and types a real question; RAG retrieves relevant chunks and answers with chapter citations', emotion: 'Trusting'  },
  ...
  'pyquest-learn-python-by-writing-it': {
    ...
    journey: [
      { phase: 'Land',      action: 'Opens PyQuest. The first thing on screen is a code editor with runnable starter code, not a syllabus. Pyodide loads quietly in the background', emotion: 'Curious'   },
  ...
  'vaani-voice-banking-agent': {
    ...
    userPersona: {
      ...
      painPoint: '"I don\'t want to sit on hold to dispute a charge or check whether my card is blocked. If I can just say it and have it actually happen, verified, confirmed and logged, that\'s the whole point."',
    },
    ...
    journey: [
      ...
      { phase: 'See it land', action: 'The netbanking dashboard refreshes on its own, so the caller watches their own balance change rather than being told it changed', emotion: 'Convinced' },
    ],
