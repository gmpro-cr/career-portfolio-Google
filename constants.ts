...
Independently designed, built, and shipped 7 products (5 LLM-based platforms and 2 browser-native learning tools, PyQuest and SQLQuest) using Claude, Cursor and Groq API.
...
export const EXPERIENCES: Experience[] = [
  {
    ...
    description: [
      ...
      "Validated borrower needs through structured discovery, identified pain points across working capital, term loans, and trade finance, and translated insights into tailored product proposals.",
  ...
  },
  {
    ...
    description: [
      ...
      "Built and maintained PowerBI and Excel dashboards tracking default probabilities, portfolio health, and loan concentration risk, used by senior management for real-time decision-making.",
...
export const PROJECTS: Project[] = [
  {
    ...
    cardSummary: "A voice agent that can move money. The model has no database access; a server-side gateway checks identity, a four-step confirmation, and policy limits first. Tested with 131 live scenarios and 189 unit tests.",
    ...
    category: 'build',
    flowType: 'voice',
    problem: "Bank IVR trees force callers through a maze of menus to do something a conversation would settle in one turn. When a call needs to actually move money, the IVR gives up and routes it to a human queue. A voice agent that can complete the transaction itself needs a way to be trusted with money without trusting the model with the database.",
    approach: [
      "Put every control that matters in a server-side gateway instead of the system prompt. The model can say anything; it can only do what dispatch() permits. Every rule the prompt states (verify identity, read back a transfer before confirming) is enforced again in code the model cannot see or persuade.",
      ...
      "Made the policy engine a pure function with no database, clock, I/O or override parameter, so a per-transaction cap, daily cap, new-payee cooling-off, and velocity rule can be reasoned about and tested in isolation from everything else.",
    ...
    keyInsights: [
      "A prompt is a request; a gateway is a guarantee. Every safety rule that actually has to hold was worth building twice: once as an instruction the model reads, and once as code it cannot negotiate with. Under pressure, only the code holds.",
      "Serious testing means reporting what broke, as well as the score after the fix. A live persona batch found a real bug that a caller could have hit: identify_caller was silently rejecting an already-authenticated session. Publishing that finding and the fix says more than a pass rate.",
      "Assertions should check the tool-call sequence and the ledger, never the wording. An agent that refuses a transfer with a different sentence every run should pass; one that states a balance it never fetched should always fail. Evals that grade phrasing miss the failures that matter.",
    ],
    outcomes: [
    ...
    technicalDetails: {
      architecture: "A caller's voice (or typed text) reaches a browser tab, which streams audio over a WebSocket to a Node relay. The relay forwards it to Gemini Live, a real-time speech-to-speech model; both directions are transcribed so an audio conversation stays auditable. When the model wants to do anything, such as check a balance or move money, it calls a named tool, and every call passes through one server-side gateway (dispatch()) that checks session validity, trust level, a confirmation token bound to the exact quoted amount, and a single-use step-up code, in that order, before touching the Postgres ledger. The gateway is transport-agnostic: the HTTP server, the voice relay, and the eval harness are all thin shells calling the same function.",
      dataFlow: [
        ...
        { step: "Every tool call routes through dispatch(), which checks session, trust level and policy before anything runs" },
  ...
  },
  {
    ...
    category: 'build',
    flowType: 'persona',
    problem: "Generic AI chatbots fail at long-term engagement because they lack character consistency and memory. Users crave authentic, persistent relationships, but foundational models inherently drift from their system prompts during extended multi-turn conversations, which breaks immersion and user trust.",
    approach: [
      ...
      "Set 'Messages Sent Per Day' as the North Star Metric because it captures functional value delivery, engagement depth, and retention signal in a single number. Supporting metrics: messages/session > 5, sessions/user/week > 2, unique personas/user > 3.",
      "Designed a prompt architecture with dynamic memory injection via Supabase, retrieving long-term context to keep personas consistent. Ran quantified persona-adherence evals, treating character consistency as a KPI and iterating on it like an A/B test variant.",
    ...
    outcomes: [
      "Grown to 500+ Monthly Active Users (DAU/MAU ~10%) driven by organic retention loops, with no paid acquisition.",
  ...
  },
  {
    ...
    cardSummary: "Nine years of writing credit memos showed the bottleneck was data extraction, not judgment. A Python engine does the math and the LLM writes the narrative, cutting CAM preparation from 4–6 hours to under 1.",
    ...
    approach: [
      "Mapped the end-to-end journey of a credit analyst to identify the exact bottleneck. Judgment takes minutes; the hours go on mechanical extraction: 40 pages of ratio calculations that Python can do in 4 seconds, and 30 tabs of web research that an AI agent can synthesise in under a minute.",
      "Built a Karpathy-style autonomous research loop: the AI agent searches Screener.in, Yahoo Finance, BSE filings, and open web for promoter pledging, rating downgrades, court cases, and industry headwinds, then scores its own knowledge completeness (0–100%) and keeps searching until it reaches 85%.",
      "Strict architectural separation: the Python engine handles all deterministic arithmetic (DSCR, D/E, ICR, 12 key ratios) and auto-flags 10 risk patterns (e.g., profits not backed by operating cash flows, surging leverage). The LLM never does the math. It writes narrative from pre-verified structured JSON.",
      "Built per-section confidence scoring (High / Medium / Low) into the output, so analysts can see exactly where the AI is uncertain, enabling targeted human review instead of full re-checks.",
    ...
    keyInsights: [
      "Trust architecture precedes feature architecture in regulated environments. The adoption bottleneck was format familiarity, not accuracy. Analysts needed AI output in the exact template their credit committee already accepted.",
      ...
      "In a regulated environment, human-in-the-loop is the core value proposition. Analysts wanted to own the final output; the AI produces the draft and the analyst makes the decision.",
    ],
    outcomes: [
    ...
    reflection: "The biggest product lesson here was that trust architecture precedes feature architecture. I assumed accuracy would be the adoption bottleneck; it turned out to be format familiarity. The moment we mirrored the exact Excel template that analysts already submitted to credit committees, resistance dropped sharply. I would have run format-discovery interviews in week one rather than week six. The second lesson: in regulated environments, human-in-the-loop is the core value proposition. Analysts didn't want to delegate decisions to the AI; they wanted the AI to produce the draft so they could own the final output. That's a fundamentally different product contract, and I'd have designed the editing UI around that insight from the start rather than treating it as a late-stage UX polish decision.",
  },
  {
    ...
    problem: "Job hunting is a high-noise, low-signal data problem. Portals are flooded with irrelevant listings, forcing candidates to manually filter hundreds of results daily. It is an inefficient, demoralising process that scales inversely with the quality of roles available.",
    approach: [
  ...
  },
  {
    ...
    approach: [
      ...
      "Designed one reusable Diagram component with 9 visual types (pipeline, loop, comparison, slider, venn, pyramid, bars, flow, curve) so the 29 diagrams across chapters stay visually consistent and are authored declaratively in each chapter's data instead of hand-built per page.",
      ...
      "Grounded every chatbot answer in retrieved chunks and forced chapter citations via Gemini 2.0 Flash, so 'Ask the book' answers from the book, with provenance, instead of hallucinating from the model's general training.",
    ...
    keyInsights: [
      ...
      "RAG quality is a retrieval problem before it's a generation problem. Forcing chapter citations and grounding strictly in retrieved chunks did more for answer trust than any prompt tuning, because users can check the source.",
    ...
    outcomes: [
      ...
      "'Ask the book' answers grounded in 1,325 retrieved chunks with chapter citations, with a fallback when the API key is absent.",
  ...
  },
  {
    ...
    date: "Jul 2026",
    image: "/pyquest.png",
    description: "A gamified Python course where real Python runs in the browser tab, with no signup, install or backend. Pyodide (CPython compiled to WebAssembly) executes every learner's code client-side, so the first print() works seconds after landing. 20 stages and 56 exercises across 3 tracks (beginner to expert), with XP, level titles, and 'Firewall' checkpoint challenges that gate progression the way a game gates levels. Deployed as a fully static site: zero servers, zero cost per learner, infinitely cacheable.",
    cardSummary: "A Python course where you write code from the first minute. Real CPython runs in the browser via Pyodide: 20 stages, 56 exercises, XP and checkpoint challenges, with nothing to install and no backend.",
    ...
    approach: [
      "Chose Pyodide (CPython compiled to WebAssembly) so learners run genuine Python instead of a lookalike interpreter. Error messages, f-strings, generators, decorators: everything behaves exactly as it will on their own machine later.",
      "Shipped the whole product as a static site. There are no accounts, server or database; progress lives in localStorage. That decision makes each additional learner literally free and removes every signup-wall drop-off from the funnel.",
      "Designed progression as a game: XP per exercise, level titles (Hatchling upward), and 'Firewall' boss challenges after each stage that must be passed to advance, which works as spaced retrieval.",
      "Structured 20 stages / 56 exercises into 3 tracks so the same product serves a first-day beginner and someone brushing up on classes and generators. The sequence is the curriculum; the tracks set the difficulty.",
      "Kept the runtime honest about loading: Pyodide is a heavyweight download, so it loads in the background with a visible status ('Python loads with your first lesson') while the learner reads stage one, so the wait is barely noticeable.",
    ],
    keyInsights: [
    ...
    outcomes: [
      ...
      "Fully static deployment on Vercel with GitHub auto-deploy: no servers to run and no per-learner cost.",
    ],
    technicalDetails: {
      architecture: "A fully static site: HTML/CSS/JS served from Vercel's CDN. Pyodide (CPython 3.x compiled to WebAssembly) is lazy-loaded in the background on first visit and cached by the browser. Exercises define expected outputs and assertion checks; learner code executes in the Pyodide runtime sandboxed inside the tab, stdout is captured and compared, and XP/progress state is written to localStorage. After the initial load it makes no network calls, so the whole course works offline.",
      dataFlow: [
  ...
  },
  {
    ...
    date: "Jul 2026",
    image: "/sqlquest.png",
    description: "An interactive SQL course where a full PostgreSQL 18 database boots inside the browser tab via PGlite (Postgres compiled to WebAssembly). 34 stages and 148 exercises take a learner from their first SELECT to window frames, query plans, CTE recursion, and row-level security. Every concept is typed, run, and answered with real rows from a real engine. No account, no install, no server. Verified by a 204-test suite that runs every exercise against the actual engine before each deploy.",
    cardSummary: "A SQL course on a real database: PostgreSQL 18 boots inside the browser tab via PGlite. 34 stages and 148 exercises from SELECT to window frames and query plans, backed by a 204-test suite that runs every exercise against the live engine.",
    ...
    approach: [
      ...
      "Scoped an ambitious curriculum deliberately: 34 stages / 148 exercises across Foundations, Aggregation & Joins, and Composition tiers, going well past most courses into materialized views, recursion, query plans, and row-level security.",
    ...
    keyInsights: [
      ...
      "The checker design shapes learning more than the lesson text. Comparing result rows (not query strings) means any correct SQL passes. Learners can find their own path to the answer, which builds confidence.",
    ],
    outcomes: [
    ...
    reflection: "SQLQuest confirmed a thesis PyQuest suggested: the WASM-ization of real engines is a product unlock for education. Once the real tool runs client-side, most of the cost and friction of teaching it disappears. What I'd do differently: I'd design the checker contract before writing a single exercise. Retrofitting row-comparison semantics (ordering, NULLs, float tolerance) across early exercises cost more than building the 148 exercises themselves. And I'd ship the test suite from exercise one. The 204 tests made content edits safe, and that is when curriculum quality started improving quickly. Next: a shared-dataset capstone tier where learners answer open analytical questions instead of guided exercises.",
  },
];
